...
@onready var jefe = $enemies/player_boss_mush
@onready var jefe2 = $enemies/player_boss_mush2
...
func _ready():
	...
	jefes_vivos = 0
	for jefe in get_tree().get_nodes_in_group("jefes"):
		jefes_vivos += 1
		if jefe.has_signal("enemigo_muerto"):
			print("Jefe conectado correctamente")
			jefe.connect("enemigo_muerto", Callable(self, "_on_jefe_muerto"))
	print("Total jefes vivos:", jefes_vivos)
