...
@onready var ani_drowned = $ani_drowned  # Animaciones de la gorgona
@onready var col_drowned = $col_drowned
@onready var audio_drowned = $audio_drowned  # Reproductor de sonido para la gorgona
...
@onready var area_ataque = $drowned_area  # Área de detección del jugador
...
func _ready() -> void:
	...
	ani_drowned.play("run")
	bar_health.visible = false
	
	# Se conectan las señales del área de ataque para detectar entrada y salida del jugador
	area_ataque.body_entered.connect(_on_drowned_area_body_entered)
	area_ataque.body_exited.connect(_on_drowned_area_body_exited)
#endregion
...
func _physics_process(delta: float) -> void:
	...
	if not is_attacking:
		...
		if sentido == 1 and detector_right.is_colliding():
			velocity.x = speed
			ani_drowned.flip_h = false
		elif sentido == -1 and detector_left.is_colliding():
			velocity.x = -speed
			ani_drowned.flip_h = true
		else:
			sentido = -sentido  # Cambia de dirección si no hay suelo

		ani_drowned.flip_h = sentido > 0
		ani_drowned.play("run")  # Reproduce la animación de correr
...
func recibir_dano(cantidad):
	...
	ani_drowned.play("hurt")
	await ani_drowned.animation_finished
	esta_herido = false
...
func morir():
	is_dead = true
	audio_drowned.play()  # Sonido de muerte
	ani_drowned.play("dead")  # Animación de muerte
	area_ataque.set_monitoring(false)  # Desactiva el área de ataque
	col_drowned.set_deferred("disabled", true)  # Desactiva el collider
	set_physics_process(false)  # Detiene el procesamiento
	emit_signal("enemigo_muerto", false)
	await ani_drowned.animation_finished  # Espera a que termine la animación
	timer_dead.start()
	await timer_dead.timeout
	queue_free()  # Elimina al ahogado de la escena
#endregion

#region Nodes Connections
# Función que se ejecuta cuando un cuerpo entra en el área de ataque
func _on_drowned_area_body_entered(body):
...
func _on_drowned_area_body_exited(body):
