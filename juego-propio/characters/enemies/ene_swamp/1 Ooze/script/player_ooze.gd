...
@onready var ani_ooze = $ani_ooze  # Animaciones del slime
@onready var col_ooze = $col_ooze
@onready var audio_ooze = $audio_ooze  # Reproductor de sonido para el slime
...
@onready var area_ataque = $ooze_area  # Área de detección del jugador
...
func _ready() -> void:
	...
	area_ataque.body_entered.connect(_on_ooze_area_body_entered)
	area_ataque.body_exited.connect(_on_ooze_area_body_exited)
#endregion
...
func recibir_dano(cantidad):
	...
	ani_ooze.play("hurt")
	await ani_ooze.animation_finished
	esta_herido = false
...
func morir():
	is_dead = true
	audio_ooze.play()  # Sonido de muerte
	ani_ooze.play("dead")  # Animación de muerte
	area_ataque.set_monitoring(false)  # Desactiva el área de ataque
	col_ooze.set_deferred("disabled", true)  # Desactiva el collider
	set_physics_process(false)  # Detiene el procesamiento
	emit_signal("enemigo_muerto", false)
	await ani_ooze.animation_finished  # Espera a que termine la animación
	timer_dead.start()
...
func _on_ooze_area_body_entered(body):
...
func _on_ooze_area_body_exited(body):
