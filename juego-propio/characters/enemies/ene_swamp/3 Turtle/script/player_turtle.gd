...
@onready var ani_turtle = $ani_turtle  # Animaciones del slime
@onready var col_turtle = $col_turtle
@onready var audio_turtle = $audio_turtle  # Reproductor de sonido para el slime
...
@onready var area_ataque = $turtle_area  # Área de detección del jugador
...
func _ready() -> void:
	...
	area_ataque.body_entered.connect(_on_turtle_area_body_entered)
	area_ataque.body_exited.connect(_on_turtle_area_body_exited)
#endregion
...
func _physics_process(delta: float) -> void:
	...
	if not is_attacking:
		...
		if sentido == 1 and detector_right.is_colliding():
			velocity.x = speed
			ani_turtle.flip_h = true
		elif sentido == -1 and detector_left.is_colliding():
			velocity.x = -speed
			ani_turtle.flip_h = false
		else:
			sentido = -sentido  # Cambia de dirección si no hay suelo

		ani_turtle.play("run")  # Reproduce la animación de correr
...
func recibir_dano(cantidad):
	...
	ani_turtle.play("hurt")
	await ani_turtle.animation_finished
	esta_herido = false
...
func morir():
	is_dead = true
	audio_turtle.play()  # Sonido de muerte
	ani_turtle.play("dead")  # Animación de muerte
	area_ataque.set_monitoring(false)  # Desactiva el área de ataque
	col_turtle.set_deferred("disabled", true)  # Desactiva el collider
	set_physics_process(false)  # Detiene el procesamiento
	emit_signal("enemigo_muerto", false)
	await ani_turtle.animation_finished  # Espera a que termine la animación
	timer_dead.start()
...
func _on_turtle_area_body_exited(body):
