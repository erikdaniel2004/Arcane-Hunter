...
@onready var ani_larva = $ani_larva  # Animaciones del slime
@onready var col_larva = $col_larva
@onready var audio_larva = $audio_larva  # Reproductor de sonido para el slime
...
@onready var area_ataque = $larva_area  # Área de detección del jugador
...
func _ready() -> void:
	...
	area_ataque.body_entered.connect(_on_larva_area_body_entered)
	area_ataque.body_exited.connect(_on_larva_area_body_exited)
#endregion
...
func recibir_dano(cantidad):
	...
	ani_larva.play("hurt")
	await ani_larva.animation_finished
	esta_herido = false
...
func morir():
	is_dead = true
	audio_larva.play()  # Sonido de muerte
	ani_larva.play("dead")  # Animación de muerte
	area_ataque.set_monitoring(false)  # Desactiva el área de ataque
	col_larva.set_deferred("disabled", true)  # Desactiva el collider
	set_physics_process(false)  # Detiene el procesamiento
	emit_signal("enemigo_muerto", false)
	await ani_larva.animation_finished  # Espera a que termine la animación
	timer_dead.start()
...
func _on_larva_area_body_entered(body):
...
func _on_larva_area_body_exited(body):
