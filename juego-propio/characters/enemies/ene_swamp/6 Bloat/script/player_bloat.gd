...
@onready var ani_bloat = $ani_bloat  # Animaciones de la gorgona
@onready var col_bloat = $col_bloat
@onready var audio_bloat = $audio_bloat  # Reproductor de sonido para la gorgona
...
@onready var area_ataque = $bloat_area  # Área de detección del jugador
...
func _ready() -> void:
	...
	area_ataque.body_entered.connect(_on_bloat_area_body_entered)
	area_ataque.body_exited.connect(_on_bloat_area_body_exited)
#endregion
...
func _physics_process(delta: float) -> void:
	...
	if not is_attacking:
		...
		if sentido == 1 and detector_right.is_colliding():
			velocity.x = speed
			ani_bloat.flip_h = false
		elif sentido == -1 and detector_left.is_colliding():
			velocity.x = -speed
			ani_bloat.flip_h = true
		else:
			sentido = -sentido  # Cambia de dirección si no hay suelo

		ani_bloat.flip_h = sentido > 0
		ani_bloat.play("run")  # Reproduce la animación de correr
...
func recibir_dano(cantidad):
	...
	ani_bloat.play("hurt")
	await ani_bloat.animation_finished
	esta_herido = false
...
func morir():
	is_dead = true
	audio_bloat.play()  # Sonido de muerte
	ani_bloat.play("dead")  # Animación de muerte
	area_ataque.set_monitoring(false)  # Desactiva el área de ataque
	col_bloat.set_deferred("disabled", true)  # Desactiva el collider
	set_physics_process(false)  # Detiene el procesamiento
	emit_signal("enemigo_muerto", false)
	await ani_bloat.animation_finished  # Espera a que termine la animación
	timer_dead.start()
...
func _on_bloat_area_body_entered(body):
...
func _on_bloat_area_body_exited(body):
